import { NextRequest, NextResponse } from "next/server";

const API_BASE_URL = process.env.BACKEND_URL;

interface BirthData {
  day: number;
  month: number;
  year: number;
  hour: number;
  min: number;
  lat: number;
  lon: number;
  tzone: number;
}

// Basic Astrology endpoints
const BASIC_ASTRO_ENDPOINTS = [
  { key: "birth_details", endpoint: "birth_details" },
  { key: "astro_details", endpoint: "astro_details" },
  { key: "planets", endpoint: "planets" },
  { key: "planets_extended", endpoint: "planets/extended" },
  { key: "bhav_madhya", endpoint: "bhav_madhya" },
  { key: "ghat_chakra", endpoint: "ghat_chakra" },
  { key: "ayanamsha", endpoint: "ayanamsha" },
];

async function fetchAPI(endpoint: string, data: BirthData) {
  try {
    const response = await fetch(`${API_BASE_URL}/api/${endpoint}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify(data),
    });

    if (!response.ok) {
      throw new Error(`API ${endpoint} failed with status ${response.status}`);
    }

    return await response.json();
  } catch (error) {
    console.error(`Error fetching ${endpoint}:`, error);
    return null;
  }
}

export async function POST(request: NextRequest) {
  try {
    const birthData: BirthData = await request.json();

    // Validate required fields
    const requiredFields = ["day", "month", "year", "hour", "min", "lat", "lon", "tzone"];
    for (const field of requiredFields) {
      if (birthData[field as keyof BirthData] === undefined) {
        return NextResponse.json(
          { status: "Fail", message: `Missing required field: ${field}` },
          { status: 400 }
        );
      }
    }

    // Validate numeric ranges
    if (
      birthData.day < 1 || birthData.day > 31 ||
      birthData.month < 1 || birthData.month > 12 ||
      birthData.year < 1900 || birthData.year > new Date().getFullYear() + 1 ||
      birthData.hour < 0 || birthData.hour > 23 ||
      birthData.min < 0 || birthData.min > 59
    ) {
      return NextResponse.json(
        { status: "Fail", message: "Invalid date/time values" },
        { status: 400 }
      );
    }

    // Validate coordinates
    if (
      birthData.lat < -90 || birthData.lat > 90 ||
      birthData.lon < -180 || birthData.lon > 180
    ) {
      return NextResponse.json(
        { status: "Fail", message: "Invalid latitude or longitude values" },
        { status: 400 }
      );
    }

    // Fetch all APIs in parallel
    const apiPromises = BASIC_ASTRO_ENDPOINTS.map((api) =>
      fetchAPI(api.endpoint, birthData).then((data) => ({
        key: api.key,
        endpoint: api.endpoint,
        data,
      }))
    );

    const results = await Promise.all(apiPromises);

    // Build response data object
    const astroData: Record<string, unknown> = {};
    for (const result of results) {
      astroData[result.key] = result.data;
    }

    // Count successful APIs
    const totalApis = BASIC_ASTRO_ENDPOINTS.length;
    const successful = results.filter((r) => r.data !== null).length;
    const failed = totalApis - successful;

    // Failed endpoints for debugging
    const failedEndpoints = results
      .filter((r) => r.data === null)
      .map((r) => r.endpoint);

    // Determine status
    let status: "Pass" | "Partial" | "Fail";
    if (successful === totalApis) {
      status = "Pass";
    } else if (successful > 0) {
      status = "Partial";
    } else {
      status = "Fail";
    }

    return NextResponse.json({
      status,
      message:
        status === "Pass"
          ? "All basic astrology data fetched successfully"
          : status === "Partial"
            ? `${successful} of ${totalApis} APIs succeeded`
            : "Failed to fetch basic astrology data",
      data: astroData,
      meta: {
        total_apis: totalApis,
        successful,
        failed,
        failed_endpoints: failedEndpoints.length > 0 ? failedEndpoints : undefined,
        success_rate: `${Math.round((successful / totalApis) * 100)}%`,
        timestamp: new Date().toISOString(),
      },
    });
  } catch (error) {
    console.error("Basic Astro API Error:", error);
    return NextResponse.json(
      {
        status: "Fail",
        message: error instanceof Error ? error.message : "Internal server error",
        data: null,
        meta: {
          total_apis: BASIC_ASTRO_ENDPOINTS.length,
          successful: 0,
          failed: BASIC_ASTRO_ENDPOINTS.length,
          success_rate: "0%",
          timestamp: new Date().toISOString(),
        },
      },
      { status: 500 }
    );
  }
}